...
Fishing::Fishing(Inventory &inv) : inventory(inv)
{
...
void Fishing::drawDropdown(Dropdown& dd, const std::vector<std::string>& options)
{
...
void Fishing::drawTemplate(float contentY) 
{
    ...
    // xp amount value
    std::string xpAmount = std::to_string(xpPerFish[itemDropdown.selectedIndex]);
    ...
    drawInfoPanel(contentY, selectedfishID, 1);
...
int Fishing::getNodeLevel(int index) const
{
    static int nodeLvls[MAX_FISHES]{1, 5, 10, 15, 20, 35, 40, 50, 50, 55, 60, 65, 70, 75, 85, 95};
    return nodeLvls[index];
}

int Fishing::getUnlockedSkillIndex() const
{
    int highest = 0;
...
void Fishing::createButton(float contentY)
{
    // button for creating the item
    Rectangle createButton{700, 580 + (contentY-100), 200 , 50};
    if (BaseSkill::rbtn(createButton, "Start Fishing"))
    {
        if (!isRunning)
        {
    ...
    {      
        ...
        progress = runningTime / fishTimer[itemDropdown.selectedIndex];
        ...
        if (runningTime >= fishTimer[itemDropdown.selectedIndex])
        {
            runningTime = 0.f;
            xpBar.width = 0.f;

            xpAccumulated = xpPerFish[itemDropdown.selectedIndex];
...
void Fishing::resetSkillProgress()
{
    BaseSkill::resetSkillProgress();
    xpBar.width = 0.f;
}

void Fishing::tick(float deltaTime, float contentY)
{
